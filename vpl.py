...
par = 1
g = 2
maxit = 2000  # Number of iterations
Leaguesize = 40
# nPlayer = 10
...
# Strategy 
class ApplyStrategy:
    def __init__(self, teams, team1, team2, Best_team,function ):
        self.teams = teams
        self.team1 = team1
        self.team2 = team2
...
# Competition
def competition(A, B, team, Best_team, function):
    ...
    r = np.random.rand()
    if d > r :
        result1 = ApplyStrategy(team, x, y, Best_team, function).result
        ...
        result2 = ApplyStrategy(team,y, x, Best_team, function).result
...
def plot_iteration_result(result,maxit,name):
    ...
    plt.legend()
    plt.savefig('E:\IVPL_SCI\picture\\function{}.png'.format(name))
    plt.pause(1)
    plt.show()    
...
if __name__ == "__main__":
    ...
    for test_set in test_set_list:
        for num in range(times):
            ...
            for itr in np.arange(maxit):
                ...
                for i in np.arange(Leaguesize - 1):
                    ...
                    for j in np.arange(Leaguesize):
                        ...
                        x, y, Best_team_ = competition(A, B, teams, Best_team, function)
            ...
            # plot_iteration_result(Best_cost_list,maxit,name)
            with open('vpl_result.csv', 'a+', newline='') as file:
